...
void Network::networkLoop()
{
	...
	if (playerSocket.bind(port2) != sf::Socket::Done)//bind socket
		{
	...
	do { //infinite loop, constantly receiving/sending data
		...
		if (playerSocket.send(packetsend, playerIP, port) != sf::Socket::Done)//send position of local player over network in packet
		{
			//error
